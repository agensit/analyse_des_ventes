...
product_dim = go.parcats.Dimension(values=df['Product'].values)
# color 
...
Parcats = go.Figure(go.Parcats(
    ...
    hoverinfo='none'))
Parcats.update_layout(title_text='5 catégories de 19 produits', title_font_size=25)
...
app.layout = dbc.Container([
	...
	# Présentation des produits
		...
		dcc.Graph(figure=Parcats, config=config_dash),
				dbc.Alert('''
			Ce diagramme ce décompose en deux colones. A gauche, on liste les differentes categories de produits vendus. 
			A droite on observe la liste des 19 produits vendus par notre entreprise d'électronique''', color='light'
		),
		dcc.Markdown('''
			Après avoir pris connaissances des differents produits, nous pouvons analyser les ventes pour __déterminer ceux qui se sont le mieux vendus en 2019__. 
			En regardant le diagramme en barres ci-dessous on constate que le macbook pro avec le plus gros chiffre de ventes.
		'''), 
		dcc.Dropdown(
			id='dropdown_type',
